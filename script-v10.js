(function(){
  ...
  async function fcTranscribeChunk(encoderSession, decoderSession, samples16k, vocabData, timeOffset){
    ...
    const logitsOutName = decOut.find(n => n.toLowerCase().includes('logit')) || decOut[0];
    const stateInNames = decIn.filter(n => !/encoder_output|target/i.test(n));
    const stateOutNames = decOut.filter(n => n !== logitsOutName);
    ...
    for (let f=0; f<numFrames; f++){
      ...
      while (count < FC_MAX_SYMBOLS_PER_FRAME){
        ...
        const targetTensor = new ort.Tensor('int32', Int32Array.from([lastToken]), [1,1]);
