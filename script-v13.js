(function(){
  ...
  // ==================================================================
  // FastConformer-Quran (RNNT) — replaces Whisper as the transcription
  // engine. Much higher reported accuracy (~0.2% WER vs Whisper's ~7-11%)
  // and architecturally simpler (a single encoder pass + a lightweight
  // per-frame greedy loop, no autoregressive text-token generation).
  // Source: voidwaveDev/fastconformer-quran (CC-BY-4.0), a browser-ready
  // mirror of mohammed/fastconformer-quran-ar-onnx-int8. Exact ONNX I/O
  // names are introspected at runtime (not hardcoded) since they weren't
  // verifiable without a live browser — everything logs with a
  // "[FastConformer]" prefix for debugging.
  // ==================================================================
  const FC_BASE = 'https://huggingface.co/voidwaveDev/fastconformer-quran/resolve/main/';
  const FC_SR = 16000, FC_N_MELS = 80, FC_N_FFT = 512, FC_WIN_LENGTH = 400, FC_HOP = 160;
  const FC_SUBSAMPLING = 8;           // encoder downsamples time by 8x
  const FC_FRAME_SECONDS = (FC_HOP * FC_SUBSAMPLING) / FC_SR; // ~0.08s per output frame
  const FC_MAX_SYMBOLS_PER_FRAME = 5; // safety cap for the inner RNNT loop
  const FC_STATE_HIDDEN = 640;        // predictor LSTM hidden size (per model card)

  let fcVocabPromise = null, fcEncoderPromise = null, fcDecoderPromise = null;
  let fcDecoderDtypes = null; // auto-detected once: { target, targetLen }
  function fcLog(...args){ console.log('[FastConformer]', ...args); }

  async function loadFcVocab(){
    if (!fcVocabPromise){
      fcVocabPromise = (async () => {
        const res = await fetch(FC_BASE + 'tokens.txt');
        if (!res.ok) throw new Error('تعذّر تحميل tokens.txt من FastConformer');
        ...
        lines.forEach((line, idx) => {
          ...
          let tok, id;
          if (sp > -1 && /^\d+$/.test(line.slice(sp+1))){
            tok = line.slice(0, sp);
            id = parseInt(line.slice(sp+1), 10);
          } else {
            tok = line; id = idx; // fallback: line position is the id
          }
          id2tok[id] = tok;
          if (/^<blk>$|^<blank>$|^<pad>$/.test(tok)) blankId = id;
        });
        if (blankId === null) blankId = lines.length - 1; // documented convention: blank is the last entry
        fcLog('vocab loaded:', Object.keys(id2tok).length, 'tokens, blank id:', blankId);
        return { id2tok, blankId };
      })();
    }
    return fcVocabPromise;
  }

  async function getFcEncoder(){
    if (!fcEncoderPromise){
      fcEncoderPromise = (async () => {
        await ensureOrt();
        fcLog('loading encoder session...');
        const sess = await ort.InferenceSession.create(FC_BASE + 'encoder.int8.onnx', { executionProviders:['wasm'] });
        fcLog('encoder inputs:', sess.inputNames, 'outputs:', sess.outputNames);
        return sess;
      })();
    }
    return fcEncoderPromise;
  }
  async function getFcDecoder(){
    if (!fcDecoderPromise){
      fcDecoderPromise = (async () => {
        await ensureOrt();
        fcLog('loading decoder/joint session...');
        const sess = await ort.InferenceSession.create(FC_BASE + 'decoder.int8.onnx', { executionProviders:['wasm'] });
        fcLog('decoder inputs:', sess.inputNames, 'outputs:', sess.outputNames);
        try{ fcLog('decoder inputMetadata:', JSON.stringify(sess.inputMetadata)); }catch(e){ fcLog('inputMetadata not available:', e.message); }
        try{ fcLog('decoder handler inputMetadata:', JSON.stringify(sess.handler ? sess.handler.inputMetadata : 'n/a')); }catch(e){}
        return sess;
      })();
    }
    return fcDecoderPromise;
  }

  // NeMo-style log-mel: 400-sample Hann window zero-padded to a 512-point
  // FFT, 160-sample hop, then per-mel-bin (per-feature) normalization
  // across the whole utterance — required despite upstream metadata
  // claiming otherwise (confirmed by the model author: without it the
  // decoder emits nothing).
  function fcComputeLogMel(samples16k){
    const window = hannWindow(FC_WIN_LENGTH);
    const fb = melFilterbank(FC_N_MELS, FC_N_FFT, FC_SR);
    const nFreqs = Math.floor(FC_N_FFT/2)+1;
    const nFrames = Math.max(1, Math.floor((samples16k.length - FC_WIN_LENGTH)/FC_HOP) + 1);
    const mel = new Float32Array(FC_N_MELS*nFrames);
    const frameBuf = new Float32Array(FC_N_FFT);
    const power = new Float32Array(nFreqs);
    for (let f=0; f<nFrames; f++){
      const start = f*FC_HOP;
      frameBuf.fill(0);
      for (let i=0;i<FC_WIN_LENGTH;i++) frameBuf[i] = (samples16k[start+i]||0) * window[i];
      for (let k=0;k<nFreqs;k++){
        let re=0, im=0;
        const w = 2*Math.PI*k/FC_N_FFT;
        for (let n2=0;n2<FC_N_FFT;n2++){ const a=w*n2; re += frameBuf[n2]*Math.cos(a); im -= frameBuf[n2]*Math.sin(a); }
        power[k] = re*re+im*im;
      }
      for (let m=0;m<FC_N_MELS;m++){
        const row = fb[m];
        let sum=0; for (let k=0;k<nFreqs;k++) sum += row[k]*power[k];
        mel[m*nFrames+f] = Math.log(sum + Math.pow(2,-24));
      }
    }
    for (let m=0;m<FC_N_MELS;m++){
      let mean=0; for (let f=0;f<nFrames;f++) mean += mel[m*nFrames+f];
      mean /= nFrames;
      let variance=0; for (let f=0;f<nFrames;f++){ const d=mel[m*nFrames+f]-mean; variance+=d*d; }
      variance /= nFrames;
      const std = Math.sqrt(variance) || 1e-5;
      for (let f=0;f<nFrames;f++) mel[m*nFrames+f] = (mel[m*nFrames+f]-mean)/std;
    }
    return { mel, nFrames };
  }

  // Greedy RNNT decode: run the encoder once, then step through its output
  // frames one at a time; at each frame, repeatedly query the decoder/joint
  // network until it emits blank (moves to next frame) or a per-frame cap
  // is hit. Each emitted token is tagged with the frame it came from, which
  // converts directly to a real timestamp — giving genuinely frame-accurate
  // word timing rather than Whisper's coarse phrase-level guesses.
  async function fcTranscribeChunk(encoderSession, decoderSession, samples16k, vocabData, timeOffset){
    const { mel, nFrames } = fcComputeLogMel(samples16k);
    const audioTensor = new ort.Tensor('float32', mel, [1, FC_N_MELS, nFrames]);
    const lengthTensor = new ort.Tensor('int64', BigInt64Array.from([BigInt(nFrames)]), [1]);
    const encIn = encoderSession.inputNames;
    const encFeeds = {};
    encFeeds[encIn[0]] = audioTensor;
    if (encIn[1]) encFeeds[encIn[1]] = lengthTensor;
    const encOut = await encoderSession.run(encFeeds);
    const encTensor = encOut[encoderSession.outputNames[0]];
    const hiddenDim = encTensor.dims[1];
    const numFrames = encTensor.dims[2];
    fcLog('encoder output dims:', encTensor.dims);

    const decIn = decoderSession.inputNames;
    const decOut = decoderSession.outputNames;
    const logitsOutName = decOut.includes('outputs') ? 'outputs' : (decOut.find(n => n.toLowerCase().includes('logit')) || decOut[0]);
    const stateInNames = decIn.filter(n => /input_states/i.test(n));
    // Pair input_states_N with output_states_N explicitly (by number),
    // ignoring any other outputs like prednet_lengths that aren't state.
    const stateOutNames = stateInNames.map(n => n.replace(/^input_states/i, 'output_states')).filter(n => decOut.includes(n));

    let states = {};
    for (const name of stateInNames){
      states[name] = new ort.Tensor('float32', new Float32Array(FC_STATE_HIDDEN), [1,1,FC_STATE_HIDDEN]);
    }

    let lastToken = vocabData.blankId; // RNNT convention: blank primes the predictor at start
    const emitted = []; // { id, frame }

    function buildFeeds(encFrameTensor, tok, dtypes){
      const targetTensor = dtypes.target==='float32'
        ? new ort.Tensor('float32', Float32Array.from([tok]), [1,1])
        : new ort.Tensor('int32', Int32Array.from([tok]), [1,1]);
      const targetLenTensor = dtypes.targetLen==='float32'
        ? new ort.Tensor('float32', Float32Array.from([1]), [1])
        : new ort.Tensor('int32', Int32Array.from([1]), [1]);
      const feeds = {};
      for (const name of decIn){
        if (/encoder_output/i.test(name)) feeds[name] = encFrameTensor;
        else if (/target_length/i.test(name)) feeds[name] = targetLenTensor;
        else if (/target/i.test(name)) feeds[name] = targetTensor;
        else feeds[name] = states[name];
      }
      return feeds;
    }

    // Auto-detect the correct dtypes for targets/target_length once, by
    // trying plausible combinations against the real first frame, instead
    // of guessing blind from error messages one field at a time.
    if (!fcDecoderDtypes){
      const frame0 = new Float32Array(hiddenDim);
      for (let c=0;c<hiddenDim;c++) frame0[c] = encTensor.data[c*numFrames + 0];
      const encFrameTensor0 = new ort.Tensor('float32', frame0, [1, hiddenDim, 1]);
      const combos = [
        {target:'float32', targetLen:'int32'},
        {target:'int32',   targetLen:'int32'},
        {target:'float32', targetLen:'float32'},
        {target:'int32',   targetLen:'float32'},
      ];
      let probeOut = null;
      for (const combo of combos){
        try{
          probeOut = await decoderSession.run(buildFeeds(encFrameTensor0, lastToken, combo));
          fcDecoderDtypes = combo;
          fcLog('auto-detected decoder dtypes:', combo);
          break;
        }catch(e){ /* try next combination */ }
      }
      if (!fcDecoderDtypes) throw new Error('تعذّر إيجاد تركيبة أنواع بيانات صحيحة لمُدخلات المُرمِّز العكسي.');
      // consume this probe result as frame 0's first step
      const logits0 = probeOut[logitsOutName];
      let bestId=0, bestVal=-Infinity;
      for (let v=0; v<logits0.data.length; v++){ if (logits0.data[v]>bestVal){ bestVal=logits0.data[v]; bestId=v; } }
      if (bestId !== vocabData.blankId){
        emitted.push({ id: bestId, frame: 0 });
        lastToken = bestId;
        stateInNames.forEach((name, i) => { if (stateOutNames[i]) states[name] = probeOut[stateOutNames[i]]; });
      }
    }

    for (let f=0; f<numFrames; f++){
      let count = (f===0 && emitted.length && emitted[emitted.length-1].frame===0) ? 1 : 0;
      while (count < FC_MAX_SYMBOLS_PER_FRAME){
        const frameSlice = new Float32Array(hiddenDim);
        for (let c=0;c<hiddenDim;c++) frameSlice[c] = encTensor.data[c*numFrames + f];
        const encFrameTensor = new ort.Tensor('float32', frameSlice, [1, hiddenDim, 1]);

        const out = await decoderSession.run(buildFeeds(encFrameTensor, lastToken, fcDecoderDtypes));
        const logits = out[logitsOutName];
        let bestId=0, bestVal=-Infinity;
        for (let v=0; v<logits.data.length; v++){ if (logits.data[v]>bestVal){ bestVal=logits.data[v]; bestId=v; } }

        if (bestId === vocabData.blankId) break;

        emitted.push({ id: bestId, frame: f });
        lastToken = bestId;
        stateInNames.forEach((name, i) => { if (stateOutNames[i]) states[name] = out[stateOutNames[i]]; });
        count++;
      }
    }
    fcLog('chunk emitted', emitted.length, 'tokens over', numFrames, 'frames');

    // Group emitted tokens into words on the SentencePiece "▁" boundary
    // marker, each word timed from its first/last constituent token frame.
    const words = [];
    let curPieces = [], curStartFrame = null, curEndFrame = null;
    function flushWord(){
      if (!curPieces.length) return;
      const raw = curPieces.join('').replace(/▁/g, ' ').trim();
      if (raw){
        words.push({
          text: raw,
          tStart: timeOffset + curStartFrame*FC_FRAME_SECONDS,
          tEnd: timeOffset + (curEndFrame+1)*FC_FRAME_SECONDS
        });
      }
      curPieces = []; curStartFrame = null; curEndFrame = null;
    }
    for (const tok of emitted){
      const piece = vocabData.id2tok[tok.id] || '';
      if (piece.startsWith('▁') && curPieces.length) flushWord();
      if (curStartFrame === null) curStartFrame = tok.frame;
      curEndFrame = tok.frame;
  ...
  async function fcTranscribeAudio(audioBuffer, onProgress){
    const vocabData = await loadFcVocab();
    const [encoderSession, decoderSession] = await Promise.all([getFcEncoder(), getFcDecoder()]);
    const data16k = await resampleBufferTo16k(audioBuffer);
    const CHUNK_SAMPLES = 20 * FC_SR; // 20s chunks keep the frame-loop cost reasonable
    const totalChunks = Math.max(1, Math.ceil(data16k.length / CHUNK_SAMPLES));
    const allWords = [];
    for (let c=0; c<totalChunks; c++){
      const start = c*CHUNK_SAMPLES;
      const chunkSamples = data16k.subarray(start, start+CHUNK_SAMPLES);
      const words = await fcTranscribeChunk(encoderSession, decoderSession, chunkSamples, vocabData, c*20);
      allWords.push(...words);
      if (onProgress) onProgress(Math.round(((c+1)/totalChunks)*100));
    }
    return allWords;
  ...
  if (transcribeBtn){
    transcribeBtn.addEventListener('click', async () => {
      ...
      try{
        ...
        const words = await fcTranscribeAudio(referenceAudioBuffer, (pct) => {
        ...
        verseTextarea.value = words.map(s=>s.text).join(' ').trim();
        computeWordAlignment();
        statusEl.textContent = words.length ? 'تم التعرّف على النص' : 'لم يُتعرّف على نص واضح في هذا المقطع';
      }catch(err){
        console.error('[FastConformer] transcription failed:', err);
